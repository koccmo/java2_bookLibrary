...
public class CategoryTwoPagingService {
    ...
    public void startPaging() {
        ...
        listOnePage = productRepository.getLimitsProductsCategoryTwo(RECORDS_COUNT_ON_PAGE, startRecordOffset);
        if ((startRecordOffset + RECORDS_COUNT_ON_PAGE) >= productRepository.getCategoryTwoCount()) {
            isLastPage = true;
        }
    }
    ...
    private void calculatePagesQuantity() {
        ...
        if ((categoryTwoCount % RECORDS_COUNT_ON_PAGE) == NO_EXTRA_PAGE) {
            pagesQuantity = (int) categoryTwoCount / RECORDS_COUNT_ON_PAGE;
        } else {
            pagesQuantity = ((int) categoryTwoCount / RECORDS_COUNT_ON_PAGE) + PAGE_OFFSET;
        }
    }
